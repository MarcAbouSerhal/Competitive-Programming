class Manacher{
    ...
    public Manacher(char[] t) {
        ...
        int l = 1, r = 1;
        for(int i = 1; i <= n; ++i){
            d[i] = Math.max(0, Math.min(r - i, d[l + r - i]));
    ...
    public final int[] maxEnding() {
        ...
        int[] res = new int[n];
        for(int i = 0; i < n; ++i)
            for(int j = Math.max(last, i); j < i + (d[(i + 1) << 1] >> 1); last = ++j)
                res[j] = 1 + ((j - i) << 1); 
        last = 0;
        for(int i = 1; i < n; ++i)
            for(int j = Math.max(last, i); j < i + (d[1 + (i << 1)] >> 1); last = ++j)
                res[j] = Math.max(res[j], (j - i + 1) << 1);
        return res;
    }
}
