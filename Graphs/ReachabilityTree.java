...
class ReachabilityTree{
    ...
    public ReachabilityTree(int n, int  m) {
        ...
        this.n = n;
        this.m = m;
        for(int i = 0; i < m; ++i) outVertex[i] = outEdge[i] = -1;
        for(int i = 0; i < n + m; ++i) dsu[i] = up[0][i] = -1;
    }
    ...
    public final int firstEdgeConnecting(int u, int v) {
        ...
        int ind = vertexIndex[v];
        for(int i = log - 1; i >= 0; --i) {
            int e = up[i][u];
            if(e != -1 && (inVertex[e - n] > ind || ind > outVertex[e - n])) u = e;
        }
        return up[0][u] - n;
    }
    // returns last edge added to component containing u (O(log(n + m)))
    public final int lastEdge(int u) { int w = find(u); return w == u ? -1 : w - n; }
    ...
    public final int[] vertexRange(int u, int e) {
        ...
        else return new int[] {inVertex[e], outVertex[e]};
    }
    ...
    public final int[] edgeRange(int u, int e) {
        e = lastEdgeBefore(u, e);
        if(e == -1) return null;
        else return new int[] {inEdge[e], outEdge[e]};
    }
    ...
    private final int find(int a) {
        while(dsu[a] != -1 && dsu[dsu[a]] != -1) a = dsu[a] = dsu[dsu[a]];
    ...
    private final void build() {
        ...
        edgeTick = 0;
        for(int i = 0; i < n + m; ++i) if(up[0][i] == -1) dfs(i);
    }
    private final void dfs(int u) {
        if(u < n) vertexPermutation[vertexIndex[u] = vertexTick++] = u;  
        else {
            int e = u - n;
